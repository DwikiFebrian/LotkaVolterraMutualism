...
# ╔═╡ 89834fd9-4da2-4b21-bfd2-9f09d3beb520
using NLsolve,  ForwardDiff, LinearAlgebra, DynamicalSystems

# ╔═╡ 215c59b9-9b19-4d25-bd66-9f4b653c9d3c
using DifferentialEquations, Plots, PlutoUI;plotly()

# ╔═╡ 39298460-30c8-11f0-0fbe-0787e39ac3aa
md"# Lotka-Volterra Mutualism Equation"

# ╔═╡ 5508463f-9e5b-432c-92f4-ea94fa40b18c
md"# Add Packages"

# ╔═╡ c10dcd3a-3003-47d9-ada4-99055cf70581
md"## Equations"

# ╔═╡ 3e60d7ae-f6e7-4f23-b584-df2d4b4e902b
md"$\begin{align}
\frac{dN_1}{dt} &= r_1N_1(1 - \frac{N_1}{K_1} + b_{12}\frac{N_2}{K_1})\\
\frac{dN_2}{dt} &= r_2N_2(1 - \frac{N_2}{K_2} + b_{21}\frac{N_1}{K_2})
\end{align}$"

# ╔═╡ d79cca5d-9abc-4341-b2ee-45ceb8823bca
md"$f(du, u, p, t)=
\begin{cases}
du[1] = p[1]u[1](1 - \frac{u[1]}{p[2]} + p[3]\frac{u[2]}{p[2]})\\
du[2] = p[4]u[2](1 - \frac{u[2]}{p[5]} + p[6]\frac{u[1]}{p[5]}])
\end{cases}$"

# ╔═╡ 9cd29992-a90b-4142-bb04-d345b1b14f99
md"## Define Julia Function"

# ╔═╡ caf5b5b8-2120-4b98-9152-6d156c77ef0e
function lotka_volterra_mutualism(du, u, p, t)
	x, y = u
	r1, k1, b12, r2, k2, b21 = p
	du[1] = dx = r1 * x*(1 - (x/k1) +b12*(y/k1))
	du[2] = dy = r2 * y*(1 - (y/k2) +b21*(x/k2))
end

# ╔═╡ e3731401-8ed8-4677-b532-9a305e6af067
md"## Assign Variables"

# ╔═╡ c6140c4a-a130-4d9f-819e-d2094ccf4a8e
md"""
nilai awal untuk Spesies 1:
	$(@bind x_begin Scrubbable(0:0.5:200, default = 100, format = ".2f"))
"""

# ╔═╡ efe5db18-e09b-4e8d-92bc-93233d6e4609
md"""
nilai awal untuk Spesies 2:
	$(@bind y_begin Scrubbable(0:0.5:200, default = 80, format = ".2f"))
"""

# ╔═╡ 1223b006-21c7-4861-ae60-5de43252e923
u_begin = [x_begin, y_begin]

# ╔═╡ 98722126-4b06-464e-b012-fff28477d4c5
md" ### Parameters"

# ╔═╡ a00102ce-7533-4cde-ac34-e23dd79d86ba
md"""
nilai awal untuk laju pertumbuhan Spesies 1:
	$(@bind r1 Scrubbable(0.1:0.01:2.0, default = 0.5, format = ".2f"))
"""

# ╔═╡ 31845922-4e6e-4684-b0d2-3bbfe7af3d24
md"""
kapasitas maksimum untuk Spesies 1 adalah:
	$(@bind k1 Scrubbable(1:2500, default = 1000))
"""

# ╔═╡ 893b5f96-5c20-4dbf-8629-23c4b97a92c7
md"""
efek mutualisme Spesies 2 untuk Spesies 1:
	$(@bind b12 Scrubbable(0.1:0.01:2.0, default = 0.2, format = ".2f"))
"""

# ╔═╡ 6308c1b8-6c25-48a9-a1e9-286131544f33
md"""
nilai awal untuk laju pertumbuhan Spesies 2:
	$(@bind r2 Scrubbable(0.1:0.01:2.0, default = 0.3, format = ".2f"))
"""

# ╔═╡ f3b2ffc4-e689-4789-8cad-46fe228cb07c
md"""
kapasitas maksimum untuk Spesies 2 adalah:
	$(@bind k2 Scrubbable(1:2500, default = 800))
"""

# ╔═╡ 7795c403-fa52-4994-af19-2de930307af6
md"""
efek mutualisme Spesies 1 untuk Spesies 2:
	$(@bind b21 Scrubbable(0.1:0.01:2.0, default = 0.15, format = ".2f"))
"""

# ╔═╡ 0ff2d087-42f8-4d47-abfd-346cc84085d2
p = [r1, k1, b12, r2, k2, b21]

# ╔═╡ 8a618f99-c106-46b5-9035-8c85d4b724b9
md"## t: Time in Years"

# ╔═╡ 110f5410-6081-40c0-81db-aaf4020e63b5
t_begin= 0

# ╔═╡ f5108d2c-67f0-40cf-84f6-11bb9ab335a4
md"## Titik Tetap"

# ╔═╡ db888e97-583a-4c23-b10b-bbba0cc2e1ce
function f!(F, x)
    N1, N2 = x
    r1, k1, b12, r2, k2, b21 = 0.5, 1000.0, 0.2, 0.3, 800.0, 0.15
    F[1] = r1 * N1 * (1 - N1/k1 + b12*N2/k1)
    F[2] = r2 * N2 * (1 - N2/k2 + b21*N1/k2)
end

# ╔═╡ 3f75ea3b-17c1-4b5f-891f-f1fcd82f0cb5
x0_list = [[100.0, 80.0], [400.0, 600.0], [600, 400], [1000.0, 1000.0]]

# ╔═╡ fbc3a9c9-98bb-4621-ae2a-4318d64935dd
results = []

# ╔═╡ c3b94967-a755-46b0-a604-895381c5cf2c
for x0 in x0_list
    sol = nlsolve(f!, x0)
	push!(results, sol.zero)
    println("Initial: $x0 → Fixed point: ", sol.zero)
end

# ╔═╡ 420c5cab-b02b-4b7d-86cd-ce5bc7462202
function f(x, y)
    dx = r1 * x * (1 - x/k1 + b12*y/k1)
    dy = r2 * y * (1 - y/k2 + b21*x/k2)
    return dx, dy
end

# ╔═╡ 7d0731bc-5d84-4065-865f-1fece743fac6
md"## Nilai Eigen dan Jacobian"

# ╔═╡ d0112165-2638-4856-9d68-bbc5d2b042c2
J = [ForwardDiff.jacobian(x -> begin
    F = similar(x)
    lotka_volterra_mutualism(F, x, p, 0.0)
    F
end, x) for x in results]

# ╔═╡ f4ec853b-c5df-458a-b18f-bfce1ace6559
md"## Jacobian yang dihitung"

# ╔═╡ f21f5a3f-383d-496e-b57a-9d622b4c1ebd
for (i, (x, jacobian_matrix)) in enumerate(zip(results, J))
    println("Jacobian at fixed point $(i): $x")
    println(Matrix(jacobian_matrix))
end

# ╔═╡ 2c29e78a-4de4-423f-8e8a-100b65d92d2a
for (i, (x, jacobian_matrix)) in enumerate(zip(results, J))
    eigvals = eigen(jacobian_matrix).values
    println("Titik tetap $i: $x")
    println("Eigenvalues: ", eigvals)
    
    if all(real.(eigvals) .< 0)
        println("Stabil")
    elseif any(real.(eigvals) .> 0)
        println("Tidak Stabil")
    else
        println("Analisis lebih lanjut")
    end
end

# ╔═╡ 80d1a0bf-d4c5-4cdc-949b-8f5e6687cef4
md"#### Didapat nilai tetap yang stabil hanya pada [1195.87, 979.38]"

# ╔═╡ df8d235f-84be-4407-9c2b-89886775094b
md"## Solve Problem "

# ╔═╡ 56af6ab3-976c-44af-a303-c0d2ee5986ba
md"Years: $(@bind t_end  Slider(0:100 , 100, true))"

# ╔═╡ 40475853-15f3-4485-bbb7-895e896bea32
t_span = [t_begin, t_end]

# ╔═╡ d062c847-ef56-41a9-b812-98f0919e9132
prob = ODEProblem(lotka_volterra_mutualism, u_begin, t_span, p)

# ╔═╡ 237520bc-95b1-42b8-8553-5ea1c44a6779
sol = solve(prob)

# ╔═╡ 078cd9ba-c213-48dd-899b-80395f20a6f6
spesies_1 = sol(t_end)[1]

# ╔═╡ c29e4f23-fac4-49f6-84ec-f5dea8ecd395
spesies_2 = sol(t_end)[2]

# ╔═╡ 2bc33473-d85e-4738-91a2-24ccbe94335f
md""" 
Populasi pada Tahun $(t_end) |
Spesies-1: $(round(Int, spesies_1)) |
Spesies-2: $(round(Int, spesies_2))
"""

# ╔═╡ 57411338-9cf2-4c67-9cc6-cd5afda4eada
plot(sol,
	legend = (0.78, 0.09),
	linewidth = 2,
	title = "Lotka-Volterra Mutualism",
	xaxis = "Waktu dalam Tahun",
	yaxis = "Jumlah Populasi",
	labels = ["Spesies-1" "Spesies-2"],
	formatter = :plain, 
	widen = true
)

# ╔═╡ fddf260d-f135-456b-bcd4-b9d709ff82ca
md"## Potret Fase"

# ╔═╡ 530571e4-7774-4018-8703-86adce9d79ae
phase = plot(sol, idxs = (1, 2),
			legend = false,
			title = "Potret Fase Lotka-Volterra",
			xaxis = "Populasi Spesies-1",
			yaxis = "Populasi Spesies-2",
			formatter = :plain,
			widen = true,
			xlims = (0.0, 1400),
			ylims = (0.0, 1100),
...
# ╔═╡ 583f66e5-7f21-42b1-a56d-c75f7642defb
scatter!(phase, (spesies_1, spesies_2), color = :red, markersize = 5)
...
manifest_format = "2.0"
project_hash = "8d31bb1ef4efcb3e015e2d86f89b6f6219dcece7"
...
git-tree-sha1 = "e9153d9bc0d232735cdc02bb54902d4983f2ad26"
...
git-tree-sha1 = "1cb6a62b39a23b24f626c00e04048a9a793082ba"
